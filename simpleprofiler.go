...
func main() {
	...
	sliceSizePtr := flag.Int("slicesize", 1024*1024, "size of single writes to file")
	numSlicesPtr := flag.Int("numslices", 1000, "number of slices of the file to write")
	...
	sliceSize = *sliceSizePtr
	numSlices = *numSlicesPtr
	fileSize = sliceSize * numSlices
	...
	/*
		If sql credentials are specified, execute the sql operations in an infinite loop: delete, write,
		and read loop. If sqlquery is specified, a loop of queries is executed instead
	*/
	if sqlCredentials != "" {
	...
	/*
		If -server is specified, open an http server that will listen for operations
		from the client: /ping (check liveness) /close (the generated file, and exit),
		/discard (read the posted data but do not write it to file) and /write (write
		the posted content to a file)
	*/
	if isServer {
	...
	/*
		If -client is specified, execute the tests: wait for the server up, write local file, send the contents
		to the server (which will discard it) and send the contents to the server (wich will write to file)
	*/
	if isClient {
		...
		// Finished
		fmt.Println("[INFO] client terminated")
	}

	// Wait here until SIGINT or SIGTERM are received
	<-doneChan
	fmt.Println("[INFO] server terminated")
}
...
func testSql(sqlCredentials string, sqlHostPort string, sqlQuery string) {
	...
	if sqlQuery != "" {
		testSqlQuery(dbHandle, sqlQuery)
		os.Exit(0)
	}
	...
	if insertThreads != 0 {
		testInsertionPerformance(dbHandle, insertThreads)
		os.Exit(0)
	}

	// Prepara the insertion/deletion/query tests
...
// Infinite loop of queries to the database
func testSqlQuery(dbHandle *sql.DB, sqlQuery string) {
	// Execute select only of query is specified
	for {
...
// Test insertion performance with the specified number of threads
func testInsertionPerformance(dbHandle *sql.DB, insertThreads int) {
